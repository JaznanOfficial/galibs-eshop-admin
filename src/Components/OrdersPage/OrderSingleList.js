...
const OrderSingleList = ({ order }) => {
    const { _id, name, img, email, payment_method, products, status, createdAt, phone } =
        order || {};
    const all_price = [];
        ...
        <tr className="border-b border-gray-200 hover:bg-gray-100">
            ...
            <td className="py-3 px-3 text-center">
                <div className="flex items-center justify-center">
                    <div className="">
                        <img className="w-6 h-6 rounded-full" src={img} alt="order-img" />
                    </div>
            ...
            <td className="py-3 px-3 text-center">
                <Link
                    to={`/orders/${_id}`}
                    className="btn btn-circle hover:bg-green-500 hover:text-white hover:border-green-500 text-green-500"
                    state={order}
                >
